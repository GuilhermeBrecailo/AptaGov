...
export const defaultBillingPlans: BillingPlanDefinition[] = [
  { code: 'STARTER', name: 'Inicial', description: 'Para começar a encontrar oportunidades', priceCents: 5_000, maxUsers: 1, maxOrganizations: 1, monthlyAlerts: 300 },
  { code: 'PRO', name: 'Profissional', description: 'Para uma operação comercial ativa', priceCents: 9_900, maxUsers: 3, maxOrganizations: 1, monthlyAlerts: 1_500 },
  { code: 'BUSINESS', name: 'Empresarial', description: 'Para equipes que precisam de escala', priceCents: 19_900, maxUsers: 10, maxOrganizations: 1, monthlyAlerts: 5_000 },
  { code: 'UNLIMITED', name: 'Ilimitado', description: 'Uso sem limite de alertas', priceCents: 39_900, maxUsers: null, maxOrganizations: null, monthlyAlerts: null },
];
...
export function parseBillingPlans(raw: string): BillingPlanDefinition[] {
  ...
  const plans = value.map((item) => {
    ...
    if (!isLimit(item.maxUsers) || !isLimit(item.maxOrganizations) || !isLimit(item.monthlyAlerts)) {
